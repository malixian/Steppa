...
function filteredPrograms() {
  ...
  return programs.filter((program) => {
    const levelMatch = activeLevel === "all" || String(program.level) === activeLevel;
...
function successRateFor(item) {
  return Math.round(statusWeight[item.status] * 100);
}
...
function renderPrograms(items) {
  ...
  programColumns.innerHTML = levels
    .map((level) => {
      ...
      const cards = levelItems.length
        ? levelItems.map(programCard).join("")
        : '<div class="status-item">没有匹配项目</div>';
      return `
        <div class="level-column">
          <div class="level-heading">
            <h3>${level.label}</h3>
            <span class="level-age">${level.age} · ${levelItems.length}项</span>
          </div>
          ${cards}
        </div>
      `;
...
function programCard(item) {
  return `
    <article class="program-card" data-status="${item.status}">
      ...
      <span class="status-pill">成功率 ${successRateFor(item)}%</span>
    </article>
  `;
}
